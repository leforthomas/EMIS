package com.geocento.webapps.earthimages.emis.common.client.places;
...
// allows the use of ValueChangeEvent.fire()
  ...
  private static String tokenPrefix = "_";
  ...
  protected final String fromGwtPlaceTokenToUrl(String gwtPlaceToken) {
    ...
    if (!isBlank(gwtPlaceToken)) {
      ...
      url = tokenPrefix + prefixValue[0] + "/" + (prefixValue.length == 2 ? URL.encodeQueryString(prefixValue[1]) : "");
    } else {
      url = initialToken;
    }
    if (isBlank(url)) {
  ...
  private String fromPathToToken(String path) {
    if (!isBlank(path)) {
      ...
      if(colon.startsWith(tokenPrefix)) {
          colon.substring(1);
      }
      return colon;
    }
  ...
  protected String getWindowLocationPath() {
      ...
      // now remove the module path from the path
      path = path.replace(getModulePath(), "");
      return path;
  }

    private String getModulePath() {
        // use the base URL to extract relative path
        String modulePath = GWT.getHostPageBaseURL();
        if(modulePath.startsWith("http://") || modulePath.startsWith("https://")) {
            modulePath = modulePath.substring(modulePath.indexOf("://") + 3);
            // get the domain out
        ...
        // remove the place information using the underscore prefix
        int index = modulePath.indexOf("/_");
        if(index != -1) {
  ...
  static public String getHostPageBaseURL() {
      // use the base URL to extract relative path
      String modulePath = GWT.getHostPageBaseURL();
      // remove the place information using the underscore prefix
      int index = modulePath.indexOf("/_");
